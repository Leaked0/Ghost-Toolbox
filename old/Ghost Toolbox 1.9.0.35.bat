...
for /f "tokens=2*" %%a in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v CurrentBuild') do set "var=%%b" >nul
if "%var%"=="19044" (
SET "build=19044.1526"
)
for /f "tokens=2*" %%a in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v CurrentBuild') do set "var=%%b" >nul
if "%var%"=="19043" (
SET "build=19043.1526"
)
for /f "tokens=2*" %%a in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v CurrentBuild') do set "var=%%b" >nul
if "%var%"=="19042" (
SET "build=19042.1526"
)
for /f "tokens=2*" %%a in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v CurrentBuild') do set "var=%%b" >nul
if "%var%"=="19041" (
SET "build=19041.1526"
)
for /f "tokens=2*" %%a in ('reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v CurrentBuild') do set "var=%%b" >nul
if "%var%"=="18363" (
...
if "%var%"=="22000" start https://bit.ly/3KaC9sC && goto begin
...
nhcolor 08 " ----------------"
color 03
aria2c\aria2c --console-log-level=warn --no-conf --file-allocation=none --check-certificate=false --continue=true --allow-overwrite=true --auto-file-renaming=false --continue=true --allow-overwrite=true --auto-file-renaming=false --http-user="rsload.net" --http-passwd="rsload.net" "https://bit.ly/3nLDbBL" -o"IOBit.Driver.Booster.v9.1.0.156.repack.zip"
timeout /t 5 >nul
cls
7z1900-extra\7za x "IOBit.Driver.Booster.v9.1.0.156.repack.zip" -aoa -pdb -oIOBit.Driver.Booster.v9.1.0.156.repack"
cls
echo. The program is being installed... Wait for the operation to complete.
timeout /t 3 >nul
cd "IOBit.Driver.Booster.v9.1.0.156.repack"
FOR %%i IN ("IObit Driver Booster*.exe") DO Set FileName="%%i"
%FileName% /SILENT
cd..
rd /s /q "IOBit.Driver.Booster.v9.1.0.156.repack" >nul 2>nul
timeout /t 2 >nul
