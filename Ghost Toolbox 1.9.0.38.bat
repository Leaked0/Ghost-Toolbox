...
nhcolor 07 "  USER: %Yellow%%USERNAME% %White%| %White%COMPUTERNAME: %Yellow%%USERDOMAIN% %White%| %White%NETFX4: %Yellow%%FNETFX4% %White%| %TPMX%%TPMVER% %White%| MD5: %HASHMD5%
...
nhcolor 07 "  CURRENT OS: %OS%%DFMT7%%CODENAME%%DFMT5%.%UBRDEC% %OSARC%%Edition%
...
for /f "tokens=2-3 delims=5. " %%i in ('wmic os get caption^,version /format:csv') do set VERSION=%%i %%j
...
set Bold=%nhcolor%[1m
cls
for /f "tokens=2 delims=," %%i in ('wmic os get caption^,version /format:csv') do set OS="%BBlue%%SWhite% %%i "
for /f "tokens=2 delims=," %%i in ('wmic os get caption^,version /format:csv') do set OSX="%%i"
...
cd "%systemdrive%\ghost toolbox\wget"
cls
wget -q --show-progress --load-cookies cookies.txt --save-cookies cookies.txt --keep-session-cookie -c --no-check-certificate "https://www.7-zip.org/a/7z2107-x64.exe" -t 5 -O "7z2107-x64.exe"
"7z2107-x64.exe" /S
cls
echo  installing....
timeout /t 2 >nul
...
cd "%systemdrive%\ghost toolbox\wget"
cls
wget -q --show-progress --load-cookies cookies.txt --save-cookies cookies.txt --keep-session-cookie -c --no-check-certificate "https://www.7-zip.org/a/7z2107.exe" -t 5 -O "7z2107.exe"
"7z2107.exe" /S
cls
echo  installing....
timeout /t 2 >nul
...
goto begin
:noupdate
for /f "tokens=2-3 delims=4. " %%i in ('wmic os get caption^,version /format:csv') do set VERSIONX=%%i %%j
set "header=GHOST MSG"
set "message2=You are using the latest build version of %VERSIONX%. Please check back again for updates at a later time."
